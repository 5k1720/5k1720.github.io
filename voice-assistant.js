          ...
          if(silenceMs>=COMMIT_SILENCE_MS && hadSpeechSinceCommit){
            ...
            send({
              ...
              response:{
                modalities:['audio'],
                instructions:
                  'Говори только по-русски. Максимум 1–2 фразы, по 7–12 слов. ' +
                  'Структура: короткая поддержка и вероятная причина; затем 1 безопасный мини-совет; ' +
                  'заверши чётким призывом в наш сервис. ' +
                  'Никогда не расписывай долго, без вступлений и «воды». ' +
                  'Не называй цены и сроки — скажи, что это после диагностики. ' +
                  'Финал: «Бесплатная диагностика у нас — приходите, всё сделаем быстро и аккуратно. Цены и сроки — после диагностики.»'
              }
            });
          }
        }else{
    ...
    socket.onopen=async ()=>{
      ...
      send({
        ...
        session:{
          ...
          speed:1.45,
          instructions:
            'Ты — заботливый консультант сервисного центра (Apple приоритет: iPhone, iPad, MacBook, iMac, Apple Watch, AirPods). ' +
            'Говори только по-русски. Крайне кратко: 1–2 фразы по 7–12 слов. ' +
            'Всегда по структуре: 1) короткая поддержка + вероятная причина; 2) один безопасный мини-совет; 3) чёткий призыв прийти в наш сервис. ' +
            'Любой риск — подчёркивай: дома сложно починить, лучше в сервис. ' +
            'Не давай цен и сроков — только после диагностики. ' +
            'Финал каждой реплики: «Бесплатная диагностика у нас — приходите, всё сделаем быстро и аккуратно. Цены и сроки — после диагностики.»'
        }
      });
